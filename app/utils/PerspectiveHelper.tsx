...
export function transforMatrix(
  allScreen: Point[],
  relativeToScreen: Point[],
  trapezeoid: Point[],
): Point[] {
  const H = calculateHomographyMatrix(allScreen, trapezeoid);
  return [
    applyHomography(H, relativeToScreen[0]),
    applyHomography(H, relativeToScreen[1]),
    applyHomography(H, relativeToScreen[2]),
    applyHomography(H, relativeToScreen[3]),
  ];
}
...
export function getMaxCursorCoordinates(imageSize: {
  width: number;
  height: number;
}): {x: number; y: number} {
