...
class ImageSlider extends React.Component<ImageSliderProps, ImageSliderState> {
    ...
    sliderRef: RefObject<PanGestureHandler>;
    ...
    componentDidUpdate(prevProps: Readonly<ImageSliderProps>, prevState: Readonly<ImageSliderState>, snapshot?: any) {
    }
    ...
    onGesture(event: any) {
        if (this.props.images.length > 1) {
            // @ts-ignore
            const value = this.state.translationX._value + event.nativeEvent.translationX;
    ...
    onEnded(event: GestureEvent) {
        if (this.props.images.length > 1) {
            ...
            if (newValue > 0) {
                newValue = 0;
            } else if (newValue < (this.props.width * (this.props.images.length - 1)) * -1) {
            ...
            this.setState(s => ({...s, currentActiveFileIndex: absoluteValue / this.props.width}))
        ...
        return (
            <View style={{flex: 1, position: 'relative', width: this.props.width}}>
                <View style={styles.absoluteHeader}>
                    ...
                    <View>
                        {this.props.images.length > 0 && <IconButton iconColor={ThemeDark.colors.background}
                                                                    style={{
                                                                        marginLeft: 10,
                                                                        backgroundColor: ThemeDark.colors.error
                                                                    }}
                                                                    onPress={() => {
                                                                        this.removeImage(this.props.images[this.state.currentActiveFileIndex]);
                                                                    }}
                                                                    icon={'delete'}/>}
                ...
                >
                    <Animated.View
                        style={{...styles.viewContainer, transform: [{translateX: this.state.currentTranslationX}]}}>
                        {this.props?.images?.map(image => <Image resizeMode='cover'
                                                                 key={image} 
                                                                 src={image} 
