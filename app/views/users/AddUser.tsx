...
function AddUser({
  ...
  const user = route.params?.user?._id
    ? useObject(User, new Realm.BSON.UUID(route.params?.user?._id))
    : null;
  const realm = useRealm();

  const [firstName, setFirstName] = React.useState(
    getOrElse(user?.firstName, ''),
  );
  ...
  const [phoneNumber, setPhoneNumber] = React.useState(
    getOrElse(user?.phoneNumber, ''),
  );
  const [placeOfResidence, setPlaceOfResidence] = React.useState(
    getOrElse(user?.placeOfResidence, ''),
  );
  ...
  function create() {
    ...
    navigation.navigate(ROUTES.USER.HOME, {
      prevEvent: PAGE_EVENTS.USER.ADD_USER_SUCCESS,
    });
  }
  ...
  function edit() {
    if (userInitialState != null) {
      ...
      reset();
      navigation.navigate(ROUTES.USER.HOME, {
        prevEvent: PAGE_EVENTS.USER.EDIT_USER_SUCCESS,
      });
    }
  }
